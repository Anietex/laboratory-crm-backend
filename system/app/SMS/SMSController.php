...
class SMSController
{
    private $usernme ="chidirafael@gmail.com";
    private $apiKey = "a601d597df1987028ecde2490e0c8d2b51b7aaeb";
    private $sms ="";
    private $url =  "http://api.ebulksms.com:8080/sendsms.json";
    private $recipients;
    private $sender = "Research";
    private $status = false;

    public function send($flash=0){
        $this->status =   $this->useJSON($this->url,$this->usernme,$this->apiKey,$flash,$this->sender,$this->sms,$this->recipients);

        return $this;
    }

    public function sendUsing( $method,$flash=0){
        switch ($method){
            case 1:
             $this->status =   $this->useJSON($this->url,$this->usernme,$this->apiKey,$flash,$this->sender,$this->sms,$this->recipients);
                break;
            case 2:
                $this->status =   $this->useXML($this->url,$this->usernme,$this->apiKey,$flash,$this->sender,$this->sms,$this->recipients);
              break;
            default:
    ...
    private  function useXML($url, $username, $apikey, $flash, $sendername, $messagetext, $recipients) {
        $country_code = '234';
        $arr_recipient = explode(',', $recipients);
